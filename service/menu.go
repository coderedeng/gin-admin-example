...
func (menuService *MenuService) getMenuTreeMap(authorityId uint) (treeMap map[string][]model.SysMenu, err error) {
	...
	err = global.GPA_DB.Where("sys_authority_authority_id = ?", authorityId).Find(&SysAuthorityMenus).Error
	if err != nil {
		return
	}
	...
	err = global.GPA_DB.Where("id in (?)", MenuIds).Order("sort").Preload("Parameters").Find(&baseMenu).Error
	if err != nil {
		return
	}
	...
	err = global.GPA_DB.Where("authority_id = ?", authorityId).Preload("SysBaseMenuBtn").Find(&btns).Error
	if err != nil {
		return
	}
...
func (menuService *MenuService) AddBaseMenu(menu model.SysBaseMenu) error {
	if !errors.Is(global.GPA_DB.Where("name = ?", menu.Name).First(&model.SysBaseMenu{}).Error, gorm.ErrRecordNotFound) {
		return errors.New("存在重复name，请修改name")
	}
	return global.GPA_DB.Create(&menu).Error
}
...
func (menuService *MenuService) getBaseMenuTreeMap() (treeMap map[string][]model.SysBaseMenu, err error) {
	...
	err = global.GPA_DB.Order("sort").Preload("MenuBtn").Preload("Parameters").Find(&allMenus).Error
	for _, v := range allMenus {
		treeMap[v.ParentId] = append(treeMap[v.ParentId], v)
	}
	return treeMap, err
...
func (menuService *MenuService) GetMenuAuthority(info *request.GetAuthorityId) (menus []model.SysMenu, err error) {
	var baseMenu []model.SysBaseMenu
	var SysAuthorityMenus []model.SysAuthorityMenu
	err = global.GPA_DB.Where("sys_authority_authority_id = ?", info.AuthorityId).Find(&SysAuthorityMenus).Error
	if err != nil {
		return
	}
	...
	err = global.GPA_DB.Where("id in (?) ", MenuIds).Order("sort").Find(&baseMenu).Error
	...
	// err = global.GPA_DB.Raw(sql, authorityId).Scan(&menus).Error
	return menus, err
}
...
func (menuService *MenuService) UserAuthorityDefaultRouter(user *model.SysUser) {
	var menuIds []string
	err := global.GPA_DB.Model(&model.SysAuthorityMenu{}).Where("sys_authority_authority_id = ?", user.AuthorityId).Pluck("sys_base_menu_id", &menuIds).Error
	if err != nil {
		return
	}
	var am model.SysBaseMenu
	err = global.GPA_DB.First(&am, "name = ? and id in (?)", user.Authority.DefaultRouter, menuIds).Error
